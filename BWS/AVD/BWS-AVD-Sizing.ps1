...
function Set-ResultsGrid {
  ...
  if ($VmPick) {
    ...
    if ($VmPick.PSObject.Properties.Name -contains 'BwsName') {
      ...
      if ($VmPick.BwsAddDisks -and @($VmPick.BwsAddDisks).Count -gt 0) {
        $rows.Add([pscustomobject]@{ Key='Additional Disks'; Value="$(@($VmPick.BwsAddDisks).Count) disk(s), CHF $($VmPick.BwsAddDiskCost)/month" })
      }
    } else {
  ...
  if ($VmPrice -and -not $HidePricing) {
    ...
    if ($VmPrice.PSObject.Properties.Name -contains 'bwsTotalPerHost') {
      ...
      $rows.Add([pscustomobject]@{ Key='Cost/Host/Month'; Value="CHF $perHost" })
      $rows.Add([pscustomobject]@{ Key='Fleet Total/Month'; Value="CHF $([Math]::Round($perHost * $Sizing.RecommendedHostsTotal, 2)) ($($Sizing.RecommendedHostsTotal) hosts)" })
...
$BtnPickVm.add_Click({
  try {
    ...
    if ($additionalDisks -and $additionalDisks.Count -gt 0) {
      $additionalDiskCost = ($additionalDisks | ForEach-Object { $_.PriceCHF } | Measure-Object -Sum).Sum
    }
      ...
      if ($isBws) {
        ...
        if ($script:LastVmPick.BwsAddDisks -and @($script:LastVmPick.BwsAddDisks).Count -gt 0) {
          foreach ($ad in @($script:LastVmPick.BwsAddDisks)) {
            $addDiskRows += "          <tr><td>Additional Disk</td><td>$($ad.Sku) ($($ad.SizeGiB) GiB, $($ad.IOPS) IOPS) CHF $($ad.PriceCHF)/mo</td></tr>`n"
          }
        }
        $priceRows = @"
...
$BtnCalcUserCosts.add_Click({
  try {
    ...
    if ($isBwsP) {
      ...
      $rows.Add([pscustomobject]@{ Key='BWS Monthly/Host'; Value="CHF $monthlyPerHost" })
    } else {
    ...
    if ($isBwsP) {
      ...
      [void]$notes.AppendLine("VM: $vmN | $hostsTotal hosts | CHF $monthlyPerHost/host/month")
    } else {
